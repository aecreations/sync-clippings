...
# General
  ...
  # Name and file
  Name "${APPNAME}"
  OutFile "SyncClippings-${APPVER}-setup-user.exe"

  # Install into user-specific folder location
  InstallDir "$LOCALAPPDATA\Programs\Sync Clippings"

  # Get installation folder from registry if available
  InstallDirRegKey HKEY_CURRENT_USER "Software\AE Creations\Sync Clippings" ""

  # Request application privileges (standard user)
  ...
  VIAddVersionKey /LANG=0 "InternalName" "SyncClippings-${APPVER}-setup-user"
  VIAddVersionKey /LANG=0 "OriginalFilename" "setup-user.nsi"
...
Section "Uninstall"
  ...
  DeleteRegKey /ifempty HKEY_CURRENT_USER "Software\AE Creations\Sync Clippings"
  DeleteRegKey HKEY_CURRENT_USER "Software\Microsoft\Windows\CurrentVersion\Uninstall\Sync Clippings Helper"

...
# General
  ...
  # Name and file
  Name "${APPNAME}"
  OutFile "SyncClippings-${APPVER}-org-setup-x64.exe"
  ...
  # Version information
  VIAddVersionKey /LANG=0 "ProductName" "Sync Clippings Helper for Organizations (64-bit)"
...
Section "Install"
  ...
  # Store app parameters for running Org Edition.
  FileOpen $3 "$INSTDIR\syncclippings-win.ini" w
  FileWrite $3 '[App Params]$\r$\n'
  FileWrite $3 'orgEdition = 1$\r$\n'
  FileWrite $3 'roamingProfile = 1$\r$\n'
  FileClose $3
  ...
  # Generate the INI file if it doesn't exist, in the roaming app data folder.
  FindFirst $1 $2 "$APPDATA\Sync Clippings\syncClippings.ini"
  StrCmp $2 "syncClippings.ini" +6 +1
  CreateDirectory "$APPDATA\Sync Clippings"
  FileOpen $5 "$APPDATA\Sync Clippings\syncClippings.ini" w
